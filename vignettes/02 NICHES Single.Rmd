...
PCHeatmap(demo,dims = 1:9,balanced = T,cells = 100)
demo <- RunUMAP(demo,dims = 1:10)
DimPlot(demo,reduction = 'umap',group.by = 'VectorType',label = F)
```
Although we can see some patterns and segregation of cell-cell relationships, this doesn't look particularly clean. This is because the four cell types that we have chosen here overlap significantly in the mechanisms that they employ. If we want to further clarify patterns that might not otherwise be obvious, we can run NICHES on the same data but imputed to amplify low-strength signal:
```{r message=F, warning=F}
...
PCHeatmap(demo.2,dims = 1:9,balanced = T,cells = 100)
